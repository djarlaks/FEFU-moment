...
int main() {
...

  {
    bool a =
        ...
        merge(std::unordered_map<char, char>(),
              std::unordered_map<char, char>()) ||
        merge(std::unordered_map<int, double>(),
...

  {
    bool a =
        ...
        merge(std::unordered_map<int, int>(),
  ...
  {
    bool a = merge(std::set<int>(), std::set<double>()) &&
...

  {
    bool a =
        ...
        merge(std::unordered_multimap<int, int>(),
...

  {
    bool a =
        ...
        merge(std::unordered_multimap<int, double>(),
              std::unordered_multimap<int, int>()) &&
        merge(std::multimap<int, double>(),
              std::unordered_multimap<int, int>()) &&
        merge(std::unordered_multimap<int, double>(),
  ...
  {
    bool a = merge(std::set<int>(), std::multiset<int>()) &&
  ...
  {
    bool a = merge(std::map<int, int>(), std::multimap<int, int>()) &&
  ...
  {
    bool a = merge(std::set<int>(), 42) && merge(42, std::set<int>()) &&
             ...
             merge(6, 66) &&
             merge(std::vector<int>{4, 2}, std::vector<int>{879});
    if (!a) {
      cout << __LINE__ << endl;
      fail();
    }
  ...
  {
    bool a = merge(std::map<int, const int>(), std::map<int, int>()) ||
             merge(std::map<int, int>(), std::map<int, const int>()) ||
             merge(std::multimap<int, volatile int>(),
                   std::unordered_map<int, int>()) ||
             merge(std::multimap<int, int>(),
                   std::unordered_map<int, volatile int>()) ||
             merge(std::multimap<int, const int>(),
                   std::unordered_map<int, volatile int>()) ||
             merge(std::multimap<int, const int>(),
                   std::unordered_map<int, const int>()) ||
             merge(std::multimap<int, volatile int>(),
                   std::unordered_map<int, volatile int>()) ||
             merge(std::multimap<int, const int>(),
                   std::unordered_map<int, const volatile int>()) ||
             merge(std::multimap<int, const volatile int>(),
                   std::unordered_map<int, const int>()) ||
             merge(std::multimap<int, const volatile int>(),
                   std::unordered_map<int, const volatile int>());
    if (a) {
...

  {
    for (int j = 0; j < 15; ++j) {
      ...
      for (const auto& p : m1) ans.insert(p);
      for (const auto& p : m2) ans.insert(p);
...

  {
    for (int j = 0; j < 15; ++j) {
      ...
      for (const auto& e : s1) ans.insert(e);
      for (const auto& e : s2) ans.insert(e);
